...
public:
	...
	void copy(const MatrixRow& origin);
	void allocate_memory(const size_t columns);
...
void MatrixRow::copy(const MatrixRow& origin)
{
	if (this == &origin)
		return;
...
void MatrixRow::allocate_memory(const size_t columns)
{
	length=columns;
	row  = new int[columns];
}
...
Matrix::Matrix(const size_t r,const size_t c)
{
	...
	width = c;
	if ((heigth != 0) && (width != 0)){
		ptr = new MatrixRow[heigth];
		for (size_t i = 0;i < heigth; i++)
			ptr[i].allocate_memory(width);
	}
	else{
		ptr=nullptr;
	}
